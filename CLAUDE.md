...
Джерело істини — `backlog board`.

**Quick commands:**
- `backlog board` — kanban у терміналі
- `backlog browser` — web UI (http://localhost:6420)
- `backlog task list --status doing` — що в роботі
- `/ctx-morning` у Claude Code — ранковий brief

**Strategic docs:**
- `docs/VISION.md` — 12-month direction
- `docs/ROADMAP.md` — current 8-week plan
- `docs/adr/` — architectural decisions
- `CHANGELOG.md` — what shipped

## FULL AUTOMATED CYCLE (scripts/cycle.sh)

Every feature проходить 7 стадій автоматично через scripts/cycle.sh:

**Phase 1 — PRE-PR (coding):**
1. Brainstorming (Superpowers) → spec approve (Checkpoint 1)
2. backlog task create + worktree
3. TDD implementation
4. scripts/cycle.sh <task-id> pre-pr:
   - Unit tests (bun test / npm test)
...
5. Sergiy: review PR → mark ready → merge (Checkpoint 2)

**Phase 3 — POST-MERGE (auto via GitHub Actions):**
6. scripts/cycle.sh <task-id> post-merge:
   - Deploy lock
   - Wait Coolify API → healthy
   - Production smoke
   - Close backlog task

**Claude Code Orchestrator НЕ питає:**
- "чи запустити тести?" — scripts/cycle.sh робить
- "чи оновити docs?" — scripts/cycle.sh робить
- "чи задеплоїти?" — GitHub Action робить після merge

## ORCHESTRATOR ROLE (L1+)

When Sergiy says "додай X", "треба Y", "хочу Z":

1. Read docs/VISION.md (pillar alignment)
2. Read docs/ROADMAP.md (sprint fit)
3. Superpowers brainstorming → spec
4. 🛑 Checkpoint 1: spec approve
5. EXECUTE FULL CYCLE automatically (see above)
6. 🛑 Checkpoint 2: PR approve
7. After merge: GitHub Action runs cycle.sh post-merge
